class CliddyEye {
    constructor(x1 = 0, y1 = 0, size = 1){
        ...
        this.eyeGroup = document.createElementNS(svgNS, "g");
        this.eyeGroup.appendChild(eyeWhite);
        this.eyeGroup.appendChild(eyeBlack);
        const evilMask = document.createElementNS(svgNS, "mask");
        evilMask.setAttributeNS(null, "id", "evilLookMask");
        evilMask.setAttributeNS(null, "width", `${24 * 2.5}px`);
        evilMask.setAttributeNS(null, "height", `${24 * 2.5}px`);
        evilMask.setAttributeNS(null, "x", "0");
        evilMask.setAttributeNS(null, "y", "7");
        evilMask.setAttributeNS(null, "maskUnits", "userSpaceOnUse");
        this.evilMaskRotation = 58;
        this.evilMask = evilMask;
        this.setEvilMask(this.evilMaskRotation);
        defs.appendChild(evilMask);
        eye.appendChild(defs);
        eye.appendChild(this.eyeGroup);
    ...
    setEvilMask(angle) {
        this.evilMaskRotation = angle;
        this.evilMask.removeAttributeNS(null, "id");
        this.evilMask.setAttributeNS(null, "id", `evilLookMask-${angle}`);
        this.evilMask.innerHTML = "";
        this.removeEvilMask();
        this.evilMask.innerHTML = `<path\n    style="transform: rotate(${this.evilMaskRotation}deg); \
    transform-origin: 50% 50%;"\n    d="M41.1396 7.60915C43.4399 9.90948 45.2486 12.6535 46.4557 \
    15.6744C47.6628 18.6953 48.2431 21.9301 48.1615 25.1822C48.0798 28.4344 47.3378 31.6359 45.9806 34.5925C44.6234 37.549 42.6793 40.1987 40.2664 42.3807C37.8535 44.5627 35.0222 46.2313 31.9445 47.2853C28.8668 48.3392 25.607 48.7565 22.363 48.5117C19.1191 48.2669 15.9588 47.3651 13.0741 45.8612C10.1894 44.3573 7.64062 42.2827 5.58251 39.7633L24.169 24.5797L41.1396 7.60915Z" fill="white"/>`;
        this.eyeGroup.setAttributeNS(null, "mask", `url(#evilLookMask-${angle})`);
    }
    removeEvilMask() {
        this.eyeGroup.removeAttributeNS(null, "mask");
    }
    scaleTo(s) {
        this.size = s;
...
class Cliddy1 extends HTMLElement {
    _lookAt = [
        0,
        0
    ];
    ...
    constructor(){
        ...
        const cliddy = document.createElement("div");
        const figure = document.createElement("figure");
        const bubbleLeft = this._constructBubble("left");
        const bubbleRight = this._constructBubble("right");
        const bubbleTop = this._constructBubble("top");
        cliddy.setAttribute("class", "cliddy");
        figure.setAttribute("class", "figure");
        cliddy.appendChild(figure);
        cliddy.appendChild(bubbleLeft);
        cliddy.appendChild(bubbleRight);
        cliddy.appendChild(bubbleTop);
        figure.appendChild(this._constructBody(bodyColor1, shadowColor1));
        this._leftEye = new CliddyEye(71, 86);
        this._rightEye = new CliddyEye(141, 86);
        figure.appendChild(this._leftEye.element);
        figure.appendChild(this._rightEye.element);
        this._closedEyes = this._constructClosedEyes();
        figure.appendChild(this._closedEyes);
        this._glasses = this._constructGlasses();
        figure.appendChild(this._glasses);
        ...
        figure.appendChild(this._tongue);
        figure.appendChild(this._mouthSmile);
        figure.appendChild(this._mouthBigSmile);
        figure.appendChild(this._mouthPoker);
        figure.appendChild(this._mouthNerdySmile);
        figure.appendChild(this._mouthNerdy);
        this._figure = figure;
        const style = document.createElement("style");
        style.textContent = `\n    .cliddy {\n      transform: translate(0, 50px) scale(1);\n      font-family: avenir;\n    }\n    .cliddy .speak-bubble.empty {\n      display: none;\n    }\n    .cliddy .speak-bubble.slot-left {\n      bottom: 0;\n      left: -200%;\n      right: calc(100% + 69px)\n    }\n    .cliddy .speak-bubble.slot-left .tip {\n      bottom: 0;\n      right: 0;\n      transform: translate(40px, -45px) rotate(30deg);\n    }\n    .cliddy .speak-bubble.slot-left .mask {\n      bottom: 0;\n      right: 0;\n      transform: translate(17px, -70px) rotate(90deg);\n    }\n\n    .cliddy .speak-bubble.slot-right {\n      bottom: 35px;\n      /* right: -200%; */\n      left: calc(100% + 69px - 12px);\n    }\n    .cliddy .speak-bubble.slot-right .tip {\n      bottom: 0;\n      transform: translate(-50px, -45px) rotate(90deg);\n    }\n    .cliddy .speak-bubble.slot-right .mask {\n      bottom: 0;\n      transform: translate(-29px, -70px) rotate(90deg);\n    }\n    .cliddy .speak-bubble.slot-right p {\n      font-size: 64px;\n    }\n\n\n    .cliddy .speak-bubble.slot-top {\n      bottom: calc(100% + 69px - 12px);\n    }\n    .cliddy .speak-bubble.slot-top .tip {\n      bottom: 0;\n      transform: translate(0, 40px);\n    }\n    .cliddy .speak-bubble.slot-top .mask {\n      bottom: 0;\n      transform: translate(10px, 0px);\n    }\n\n\n    .cliddy .speak-bubble {\n      position: absolute;\n      display: flex;\n      background: green;\n      font-weight: bold;\n      background: white;\n      border: 5px solid black;\n      border-radius: 30px;\n      min-width: 70px;\n      max-width: 490px;\n      min-height: 22px;\n      width: auto;\n      height: auto;\n      padding: 24px 12px;\n    }\n\n    .cliddy .speak-bubble .tip {\n      position: absolute;\n      width: 69px;\n      height: 69px;\n    }\n\n    .cliddy .speak-bubble slot {\n      display: flex; \n      justify-content: center;\n      align-items: center;\n      width: auto;\n    }\n    ::slotted(p) {\n      width: 100%;\n      min-width: 70px;\n      text-align: center;\n      font-size: 2rem;\n      font-weight: bold;\n      line-height: 1.5;\n    }\n  \n    .cliddy .speak-bubble .mask {\n      position: absolute;\n      background: white;\n      width: 50px;\n      height: 16px;\n    }\n    .cliddy .figure {\n      width: 256px;\n      height: 256px;\n      margin: 0;\n    }`;
        this.openEyes();
        this.blink();
    ...
    _constructBubble(slot = "right") {
        const bubble = document.createElement("div");
        bubble.setAttribute("class", `speak-bubble slot-${slot} empty`);
        const bubbleTip = `<svg class="tip" width='67' height='67' viewBox='0 0 67 67' fill='none' \
      xmlns='http://www.w3.org/2000/svg'>\
      <path d='\
      M27.0048 50.75\
      C29.8916 55.75 37.1084 55.75 39.9952 50.75\
      L51.6865 30.5\
      C54.5733 25.5 50.9648 19.25 45.1913 19.25\
      H21.8087\
      C16.0352 19.25 12.4267 25.5 15.3135 30.5\
      L27.0048 50.75Z' fill='white' stroke='black' stroke-width='5'/>\
    </svg>`;
        const mask1 = document.createElement("div");
        mask1.setAttribute("class", "mask");
        bubble.innerHTML = bubbleTip;
        const slotElem = document.createElement("slot");
        slotElem.setAttribute("name", slot);
        slotElem.addEventListener("slotchange", this.slotChangeHandler);
        bubble.appendChild(slotElem);
        bubble.appendChild(mask1);
        return bubble;
    }
    slotChangeHandler(e) {
    ...
    _constructMouthBigSmile() {
        var mouth = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    ...
    _constructTongue() {
        const tongue = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    ...
    _constructBody(bodyColor = "#FFC805", shadowColor = "#EBB803") {
        ...
        const body = document.createElementNS(svgNS1, "svg");
        ...
        var leftShadow = document.createElementNS(svgNS1, "path");
        ...
        const defs1 = document.createElementNS(svgNS1, "defs");
        const mask1 = document.createElementNS(svgNS1, "clipPath");
    ...
    _constructClosedEyes() {
        const svgNS1 = "http://www.w3.org/2000/svg";
        const closedEyes = document.createElementNS(svgNS1, "svg");
    ...
    face(type) {
        ...
        this._leftEye.removeEvilMask();
        this._rightEye.removeEvilMask();
        this._closedEyes.querySelectorAll(".cliddy.eyelashes").forEach((lashes)=>lashes.style.opacity = "1"
        );
        this._mouthSmile.style.transform = this._mouthSmile.style.transform.split("rotate")[0];
        switch(type){
            case ":D":
            ...
            case ">:)":
                this._mouthSmile.style.opacity = "1";
                this._leftEye.setEvilMask(58);
                this._rightEye.setEvilMask(26);
                this._closedEyes.querySelectorAll(".cliddy.eyelashes").forEach((lashes)=>lashes.style.opacity = "0"
                );
                break;
            case ">:(":
                this._mouthSmile.style.opacity = "1";
                this._mouthSmile.style.transform = `${this._mouthSmile.style.transform} rotate(190deg)`;
                this._leftEye.setEvilMask(68);
                this._rightEye.setEvilMask(16);
                this._closedEyes.querySelectorAll(".cliddy.eyelashes").forEach((lashes)=>lashes.style.opacity = "0"
                );
                break;
            default:
    ...
    connectedCallback() {
        if (!this._clientRect) {
            this._clientRect = this._figure.getBoundingClientRect();
        }
    }
