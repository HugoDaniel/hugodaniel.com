...
description = "WebGPU wiring you can ship and share"
date = 2026-08-23
extra = { place = "Amadora", author = "Hugo Daniel", social_img = "/images/pngine.png", class = "narrow-media", modules = ["modules/pngine-player.js"], related = ["posts/2026-06-02-sjon.md", "posts/2026-08-07-wgslender-my-wgsl-toolchain.md", "posts/2025-12-31-introducing-pngine.md", "posts/2026-05-25-s-rausch.md"] }
+++
...
The code above is a simple red triangle done in WebGPU using S-expressions for the plumbing that match 1:1 with the [WebGPU spec](https://www.w3.org/TR/webgpu/).
...
Read more about all of this on its [dedicated page here](https://pngine.com).

The code is CC0 and is available in [Github for issues and discussions](https://github.com/HugoDaniel/pngine) and the releases are cut from my self-hosted repo.

![pngine logo](/images/pngine.png)
...
[SJON](/pages/sjon) gave me a way to build DSLs that can be automatically validated, and delivered in a consistent composable way through S-expressions. This is just for the WebGPU wiring and parts, not for the shader code, WGSL I see as a DSL of its own and is kept as is, pristine, not extended. 
...
I won't go much further into this topic because I have written extensively about this in the [SJON post](https://hugodaniel.com/posts/sjon/) and [page](https://hugodaniel.com/pages/sjon/).
...
_(btw the JavaScript code was generated with pngine, it is one possible output from the s-expressions)_
...
Sharing shader code is neat, [shadertoy](https://www.shadertoy.com/) is an amazing tool with an even better community, lots of examples and code to learn from, just impressive. Use it.

If you want to go a bit deeper, there is [compute.toys](https://compute.toys/), it allows you to have multiple compute shaders that feed each other into a final buffer that gets sent to the screen as pixels (ok, its a bit more than this, but just to exemplify). It extends WGSL with #macros that you can use to specify a few pipeline configurations and what not. Amazing tool, I have done a lot of my work in it, it is great if you want to learn WGSL and play around with some ideas.
...
<figure class="pngine-player">
  <div class="pngine-stage" style="background-image:url('/images/pngine-particle-fountain.png'); aspect-ratio: 640 / 400;">
    <canvas width="640" height="400" aria-label="Particle fountain, live WebGPU render"></canvas>
  </div>
  <figcaption class="pngine-bar">
    <button type="button" class="pngine-toggle" aria-pressed="false" data-src="/images/pngine-particle-fountain.png">Play</button>
    <span class="pngine-status" role="status"></span>
    <a class="pngine-download" href="/images/pngine-particle-fountain.png" download="particle-fountain.png" title="This PNG is the program: its pixels are one rendered frame, its pNGb chunk the bytecode and the runtime">Download the PNG</a>
  </figcaption>
</figure>

<style>
.pngine-player { margin: 2rem auto; max-width: min(28rem, 100%); border: 1px solid #d5d5d8; border-radius: 8px; overflow: hidden; background: #050508; }
.pngine-stage { position: relative; background-color: #050508; background-position: center; background-size: cover; background-repeat: no-repeat; }
.pngine-stage canvas { display: block; position: absolute; inset: 0; width: 100%; height: 100%; touch-action: none; }
.pngine-bar { display: flex; margin: 0; align-items: center; gap: 0.75rem; padding: 0.5rem 0.75rem; border-top: 1px solid #d5d5d8; background: #f6f6f7; font-size: 0.85rem; line-height: 1.4; }
.pngine-toggle { flex: none; min-width: 5.5rem; padding: 0.35rem 0.9rem; border: 1px solid #b9773f; border-radius: 999px; background: #C75D3A; color: white; font: inherit; font-weight: 600; cursor: pointer; }
.pngine-toggle[aria-pressed="true"] { background: #55555c; border-color: #55555c; }
.pngine-toggle:disabled { cursor: default; opacity: 0.5; }
.pngine-status { flex: 1 1 auto; color: #666; }
.pngine-status:empty { display: none; }
.pngine-download { margin-left: auto; flex: none; color: #666; font-size: 0.8rem; }
@media (prefers-color-scheme: dark) {
  .pngine-player { border-color: #3a3a40; }
  .pngine-bar { background: #26262a; border-top-color: #3a3a40; }
  .pngine-status, .pngine-download { color: #999; }
}
@media (max-width: 30rem) { .pngine-bar { flex-wrap: wrap; } }
</style>

2048 GPU particles with buffer pooling, shot up from a nozzle, pulled down by
gravity, and then respawned with a fresh random velocity when their life runs out.  

In this example the attribute buffer is also the thing that the simulation writes.

The full document is on the
[particle fountain sample page](/pages/pngine/docs/samples/particle-fountain/).
Or take the PNG above: it is the program, poster and all.

### Particle fountain, and then...

<figure class="pngine-player">
  <div class="pngine-stage" style="background-image:url('/images/pngine-instanced-trees.png'); aspect-ratio: 640 / 400;">
    <canvas width="640" height="400" aria-label="Instanced trees, live WebGPU render"></canvas>
  </div>
  <figcaption class="pngine-bar">
    <button type="button" class="pngine-toggle" aria-pressed="false" data-src="/images/pngine-instanced-trees.png">Play</button>
    <span class="pngine-status" role="status"></span>
    <a class="pngine-download" href="/images/pngine-instanced-trees.png" download="instanced-trees.png" title="This PNG is the program: its pixels are one rendered frame, its pNGb chunk the bytecode and the runtime">Download the PNG</a>
  </figcaption>
</figure>
...
the other per instance, so the whole forest is `(draw :vertex-count 12
:instance-count NUM_TREES)`.

Check out the full document on the
[instanced trees sample page](/pages/pngine/docs/samples/instanced-trees/).
As before, the program, poster and all is the PNG too here.
...
Check out [pngine here](https://pngine.com), and/or [see the code here](https://github.com/HugoDaniel/pngine).
